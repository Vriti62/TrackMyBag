...
const InsurancePaymentPage = () => {
  ...
  const handlePaymentSuccess = async () => {
    try {
      console.log('Attempting to update insurance status...'); // Debug log
      ...
      const response = await fetch('http://localhost:3000/api/users/update-insurance', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to update insurance status');
      }

      const data = await response.json();
      console.log('Insurance status updated:', data);
      
      setMessage('Insurance status updated successfully!');
      setTimeout(() => {
      ...
      setMessage(error.message || 'Failed to update insurance status');
    }
  };
